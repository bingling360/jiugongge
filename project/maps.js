var maps_90f36752_8815_4be8_b32b_d7fad1d0542e = 
{
	...
	"121": {"cls":"npcs","id":"man","name":"伯伯","event":[{"type":"if","condition":"(status:atk >=9)","true":[{"type":"choices","text":"\t[伯伯,man]要加6防、减9攻吗？","choices":[{"text":"同意","action":[{"type":"setValue","name":"status:def","operator":"+=","value":"6"},{"type":"setValue","name":"status:atk","operator":"-=","value":"9"},{"type":"function","function":"function(){\ncore.removeBlock(core.nextX(), core.nextY());\n}"},{"type":"tip","text":"交易成功！防御+6，攻击-9"}]},{"text":"不同意","action":[]}]}],"false":[{"type":"tip","text":"攻击不足，无法交易"}]}]},
	"122": {"cls":"npcs","id":"trader","name":"商人","event":[{"type":"if","condition":"(status:def >=9)","true":[{"type":"choices","text":"\t[商人,trader]要加6攻、减9防吗？","choices":[{"text":"同意","action":[{"type":"setValue","name":"status:atk","operator":"+=","value":"6"},{"type":"setValue","name":"status:def","operator":"-=","value":"9"},{"type":"function","function":"function(){\ncore.removeBlock(core.nextX(), core.nextY());\n}"},{"type":"tip","text":"交易成功！攻击+6，防御-9"}]},{"text":"不同意","action":[]}]}],"false":[{"type":"tip","text":"防御不足，无法交易"}]}]},
	"123": {"cls":"npcs","id":"thief"},
	"124": {"cls":"npcs","id":"fairy","name":"仙子","event":[{"type":"if","condition":"(flag:fairyRotChoice == 1)","true":[{"type":"tip","text":"你已经进行了选择"}],"false":[{"type":"choices","text":"\t[仙子,fairy]实验性功能：跨面旋转功能可能会出现 bug，是否要开启？开启后跨面移动时画面会随之旋转。","choices":[{"text":"开启","action":[{"type":"function","function":"function(){\nif (core.plugin.cubeWorld && core.plugin.cubeWorld.setCrossViewRotationEnabled) {\n\t\tcore.plugin.cubeWorld.setCrossViewRotationEnabled(true);\n\t}\n}"},{"type":"setValue","name":"flag:fairyRotChoice","value":"1"}]},{"text":"不开启","action":[{"type":"function","function":"function(){\nif (core.plugin.cubeWorld && core.plugin.cubeWorld.setCrossViewRotationEnabled) {\n\t\tcore.plugin.cubeWorld.setCrossViewRotationEnabled(false);\n\t}\n}"},{"type":"setValue","name":"flag:fairyRotChoice","value":"1"}]}]}]}]},
	"125": {"cls":"npcs","id":"wizard"},
	"126": {"cls":"npcs","id":"recluse","name":"隐士","event":[{"type":"choices","text":"\t[隐士,recluse]将身上的黄钥匙、飞行器（中心对称）、蓝钥匙、炸弹、跳跃鞋、破墙镐交予我，我便为你削减铃兰花妖与郁金香花妖的生命值。每枚黄钥匙或飞行器减1%、每枚蓝钥匙/炸弹/跳跃鞋减2%、每把破墙镐减4%（与光环叠加，最低保留20%血量）。另可献祭红钥匙：每2把减15%。是否献上？","choices":[{"text":"献上这些道具","action":[{"type":"function","function":"function(){\n  var c = function (id) { return core.itemCount(id); };\n  var P = 0;\n  P += 1 * (c('yellowKey') + c('centerFly'));\n  P += 2 * (c('blueKey') + c('bomb') + c('jumpShoes'));\n  P += 4 * c('pickaxe');\n  P = Math.round(P * 100) / 100;\n  var prev = core.getFlag('recluseDeductP', 0) || 0;\n  core.setFlag('recluseDeductP', prev + P);\n  var consumeIds = ['yellowKey', 'centerFly', 'blueKey', 'bomb', 'jumpShoes', 'pickaxe'];\n  consumeIds.forEach(function (id) {\n    var n = core.itemCount(id);\n    if (n > 0) core.removeItem(id, n);\n  });\n  core.updateStatusBar();\n  if (core.plugin && core.plugin.cubeWorld) core.plugin.cubeWorld.refreshDamage();\n  if (P > 0) core.drawTip('隐士收下了你的道具，铃兰花妖与郁金香花妖的生命值削减了 ' + P + '%。', 'recluse');\n  else core.drawTip('你身上没有可献祭的道具。', 'recluse');\n}"}]},{"text":"献祭红钥匙（每2把减15%）","action":[{"type":"function","function":"function(){\n  var n = core.itemCount('redKey');\n  var pairs = Math.floor(n / 2);\n  if (pairs <= 0) { core.drawTip('你身上的红钥匙不足2把，无法献祭。', 'recluse'); return; }\n  var P = 15 * pairs;\n  P = Math.round(P * 100) / 100;\n  var prev = core.getFlag('recluseDeductP', 0) || 0;\n  core.setFlag('recluseDeductP', prev + P);\n  core.removeItem('redKey', pairs * 2);\n  core.updateStatusBar();\n  if (core.plugin && core.plugin.cubeWorld) core.plugin.cubeWorld.refreshDamage();\n  core.drawTip('隐士收下了你的' + (pairs * 2) + '把红钥匙，铃兰花妖与郁金香花妖的生命值削减了 ' + P + '%。', 'recluse');\n}"}]},{"text":"再考虑一下","action":[]}]}]},
	"127": {"cls":"npcs","id":"king"},
	"128": {"cls":"npcs","id":"youngMan"},
	"129": {"cls":"npcs","id":"sign"},
	"130": {"cls":"npcs","id":"expShop","name":"经验商店"},
	"131": {"cls":"npcs","id":"moneyShop","name":"金币商店"},
	...
	"161": {"cls":"terrains","id":"arrowUp","canPass":true,"cannotOut":["left","right","down"],"cannotIn":["up"],"faceIds":{"up":"arrowUp","right":"arrowRight","down":"arrowDown","left":"arrowLeft"}},
	"162": {"cls":"terrains","id":"arrowDown","canPass":true,"cannotOut":["left","right","up"],"cannotIn":["down"],"faceIds":{"up":"arrowUp","right":"arrowRight","down":"arrowDown","left":"arrowLeft"}},
	"163": {"cls":"terrains","id":"arrowLeft","canPass":true,"cannotOut":["up","down","right"],"cannotIn":["left"],"faceIds":{"up":"arrowUp","right":"arrowRight","down":"arrowDown","left":"arrowLeft"}},
	"164": {"cls":"terrains","id":"arrowRight","canPass":true,"cannotOut":["up","down","left"],"cannotIn":["right"],"faceIds":{"up":"arrowUp","right":"arrowRight","down":"arrowDown","left":"arrowLeft"}},
